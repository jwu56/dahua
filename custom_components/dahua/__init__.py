...
class DahuaDataUpdateCoordinator(DataUpdateCoordinator):
        ...
        if not self.initialized:
            try:
                ...
                if(is_nvr):                    
                    ...
                    self.ip_with_smallest_final_octet = self.resolve_ips()
                    self._current_device_discovery_id = self.get_index_of_device_in_discovery()
    ...
    def get_current_device_sn(self) -> str:
        return self.nvr_devices_discovery["deviceInfo[{}].SerialNo".format(self._current_device_discovery_id)]
    def get_current_device_model_number(self) -> str:
        return self.nvr_devices_discovery["deviceInfo[{}].DeviceType".format(self._current_device_discovery_id)]
    def get_current_device_machine_name(self) -> str:
        return self.nvr_devices_discovery["deviceInfo[{}].MachineName".format(self._current_device_discovery_id)]
    ...
    def resolve_ips(self) -> str:
        ...
        for i in range (1,len(self._nvr_devices_discovery)):
            ...
            ip_octets = self.nvr_devices_discovery["deviceInfo[{}].IPv4Address.IPAddress".format(i)].split(".")
